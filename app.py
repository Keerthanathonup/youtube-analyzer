...
@app.get("/network")
async def network_view(request: Request, db: Session = Depends(get_db)):
    """Display the video relationship network visualization."""
    repo = VideoRepository(db)
    
    # Get recent videos to populate the initial network view
    recent_videos = repo.get_recent_videos(limit=10)
...
@app.get("/network/{video_id}")
async def network_view_for_video(request: Request, video_id: str, db: Session = Depends(get_db)):
    """Display the network visualization centered on a specific video."""
    repo = VideoRepository(db)
    
    # Get the central video
    video = repo.get_video_by_id(video_id)
    if not video:
        return templates.TemplateResponse(
            "error.html",
            {"request": request, "error": "Video not found"}
        )
    
    # Get network data
    network_data = repo.get_video_network(video_id, depth=2, max_videos=20)
...
@app.get("/api/network/{video_id}")
async def api_get_network(video_id: str, db: Session = Depends(get_db)):
    """API endpoint to get network data for a video."""
    try:
        repo = VideoRepository(db)
        ...
        # Get related videos
        related_videos = repo.get_related_videos(video_id, limit=10)
        
        # Create a list including the central video and related videos
        all_videos = [video] + related_videos
        
        # Generate a simple network structure
        nodes = [
            {
                "id": v.id,
                "title": v.title,
                "channel": v.channel_title,
                "group": 1 if v.id == video_id else 2  # Central node is group 1
            }
            for v in all_videos
        ]
        ...
        edges = [
            {
                "source": video_id,
                "target": v.id,
                "value": 1  # Simple equal weighting for now
            }
        ...
        print(traceback.format_exc())
        return {"error": f"Error generating network: {str(e)}"}